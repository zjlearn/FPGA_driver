...
static irqreturn_t fpga_interrupt(int irq, void *dev_id,struct pt_regs *regs) //delete the parameter of  struct pt_regs *regs
{
   ...
   printk(KERN_INFO "write_finished ! ~~~~~~~~~~, the result buffer num is %d, the device is %s \n", result_buffer->num,fpga->devicename);
    ...
    /*for test we should delete it after test it influence the right of result*/
    /*copy the task messages into the result */
...
void sendTask(struct  fpga_device * fpga){
   ...
   // printk(KERN_INFO "enter the send task function! ******\n");    
    ...
    {
        printk(KERN_EMERG "failed get the tx_sem, the device is %s \n", fpga->devicename);
         wake_up_interruptible(&fpga->inq);  
        return ; 
    } 
    if(down_trylock(& (fpga->rx_sem)))
    {
       printk(KERN_EMERG  "failed get the rx_sem the device is %s \n", fpga->devicename);
...
ssize_t fpga_read(struct file *filePtr, char __user *buf, size_t count, loff_t *pos){
    ...
    while(find_rx_buffer(fpga,1)== NULL ){  //没有本进程的数据可以读
        up(&fpga->rx_sem);
       // printk(KERN_EMERG " \" %s\" reading : going to sleep\n", current->comm);
    ...
    if(rx_buffer ==NULL ){
        printk(KERN_EMERG "there is some wrong in the fpga_read function!\n");
         up(&fpga->rx_sem);
        return -EFAULT;
    }
    ...
    if(copy_to_user(buf,rx_buffer->dma_buffer_rx,count)){ //copy the data to the user directly
        up(&fpga->rx_sem);
        printk(KERN_EMERG "the copy_to_user is some thing wrong！###################\n");
        return -EFAULT;
    }
...
ssize_t fpga_write(struct file *filePtr, const char __user *buf, size_t count, loff_t *pos){
   ...
   // printk(KERN_INFO "the current processs is %s",current->comm);
    ...
    printk(KERN_INFO "the process %d has write the data into the %d num, the device is %s \n ", current->tgid,tx_buffer->num, fpga->devicename);
...
int  sendTask_thread(void * data ){
    while(1){
     ...
     printk(KERN_INFO "rnter thread\n");
    if(down_trylock(& (fpga->tx_sem)))  //we only try to lock the sem
    {
        //printk(KERN_EMERG "failed get the tx_sem\n");
        goto outtwo;
    } 
    if(down_trylock(& (fpga->rx_sem)))
    {
       // printk(KERN_EMERG  "failed get the rx_sem\n");
        up(&(fpga->tx_sem));  //解除申请的tx_sem锁。
        goto outtwo;
    }
    struct tx_buffer_struct * task=NULL;
   ...
   // printk(KERN_INFO "get the buffer lock !\n");
    ...
    if( ! fpga->write_finished){
        //printk(KERN_INFO "NO ready!\n");
        goto outone;
    }
   // printk(KERN_INFO "the task in the buffer is %d \n, device is %s \n ", fpga->write_num - fpga->send_num, fpga->devicename);
   ...
   //printk(KERN_INFO "the task is to send in the %d tx_buffer! the device is %s \n", task->num, fpga->devicename);
