...
void * getResult(void *m ){
    ...
    int fd=((THREAD_PARAM *)m)->_device;
    unsigned char  outbuf[43520];
    for(i=0; i<NUM; i++){
     len=read(fd,outbuf,43520); //the 234 is 
      if(len<0)
...
int main()
{
    void print_msg(void*);
    int fd;
    fd=open("/dev/mapfpga2",O_RDWR);
    pthread_t t1,t2;
    
    FILE  *fp_result=fopen("map_key80.txt","w");
    FILE *fp_data=fopen("map_data_int.txt","r");
